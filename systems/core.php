<?php
    ...
    function globals_params($prefix = '', $run = false){
        foreach ($_GET as $var => $value) {
            if ($run || !$GLOBALS[$prefix.'_'.$var]) $return[$prefix.'_'.$var] = $value;
        }
    ...
    // временная переменная, не учитывает серверное окружение
    function getStaticsDomain() {
        return 'http://st.examole.com/';
    }

    function getTrailersDomain() {
        return 'http://tr.example.com/';
    }
